...
// srd-3-escaping R3.2 requires the front-matter templates and the node
...
// escaper holds the replacement table srd-3-escaping R1.1 states: the ten
...
// other character as written (srd-3-escaping R1.1, R1.2, R1.3, R2.1).
//
...
// claims to protect (srd-3-escaping R2.2, R2.3).
//
// The function takes no options and returns no error, so it cannot behave one
// way for a title and another for a paragraph (srd-3-escaping R3.1).
